...
ReconPlanner.run(domain, requirement)
  ...
  → loop:
      ...
      → if DONE and [feature-gated] Verification Subagent 启用:
          spawn Verification Subagent → VERDICT
          if FAIL/PARTIAL: 覆盖 DONE，gaps 进入下一轮 briefing
      → [可选] spawn Research Subagent（按需深入调研）
      → generate_briefing(wm, direction, [research_reports], [verification_gaps])
  → 编译最终报告
```
...
| `VERIFICATION_SUBAGENT_ENABLED` | 否 | Verification Subagent 开关，默认 `true`。设为 `false` 禁用后 evaluate_and_decide 的 DONE 直接生效 |
...
### Step 5：ReconPlanner + Research Subagent + Verification Subagent
...
- Session 结束后：
  ...
  3. **[feature-gated] spawn Verification Subagent**：仅在 evaluate_and_decide 判 DONE 时触发。VERDICT == PASS 才真正 DONE，否则覆盖为 CONTINUE 并把 gaps 塞进 briefing。
  4. **generate_briefing**（Planner LLM 调用）：从 WM + research 报告 + verification gaps 生成下一轮 briefing。
...
**Verification Subagent（tool-use 循环，DONE 守门人，feature-gated 可拆卸）：**
- **可拆卸**：通过 `VERIFICATION_SUBAGENT_ENABLED` 环境变量控制，禁用时 evaluate_and_decide 的 DONE 直接生效（原行为）
- 工具：`read_world_model(location?)`, `bash(command)`, `think(thought)` — **只读 + 执行**
- 输入：WM 快照 + requirement + DONE 理由 + session transcript 摘要
- 输出：验证报告写入 `artifacts/{domain}/verification/round_N.md`，最后一行必须是 `VERDICT: PASS / FAIL / PARTIAL`
- 触发时机：**只在 evaluate_and_decide 判 DONE 时**（CONTINUE 时不跑，避免每轮成本）
- 权力：verdict 可以**覆盖 DONE**——这是反 satisficing 的关键
...
**停止条件：**
1. evaluate_and_decide 返回 DONE **且** Verification Subagent verdict == PASS（或 feature 禁用时 DONE 直接生效）
