...
├── runtime/                       # Runtime services（agent 不可见的可换实现）
│   └── human_assist.py        # HumanAssistGateway 接口 + TerminalGateway 实现
├── recording/
...
# 浏览器持久化 profile（独立于 mission 数据,跨 run 复用）
artifacts/_profiles/{domain}/  # Camoufox/Firefox user_data_dir，cookies/localStorage/IndexedDB 全在这
```
...
**浏览器连接优先级：**
1. 默认 → `AsyncCamoufox(persistent_context=True, user_data_dir=artifacts/_profiles/{domain}/)` 直接启动
2. `BROWSER_WS_URL` 环境变量 → `playwright.firefox.connect(ws_url)`（远程 Camoufox，无 profile 持久化）
3. `BROWSER_CDP_URL` 环境变量 → `playwright.connect_over_cdp(url)`（远程 Chromium）
4. Camoufox 不可用 → `playwright.chromium.launch()` 本地 Chromium 兜底（无 profile 持久化）

**Camoufox 默认参数（MVP 实测稳定）：**
- `window=(1280, 900)`, `viewport={"width":1280,"height":816}` — 适配 Windows 11 @ 150% 缩放
- `os="windows"` — 匹配宿主 OS,避免 Linux/macOS 渲染 quirks
- `humanize=True` — 鼠标轨迹合成,轻度 behavioral 改善
- `persistent_context=True` + per-domain `user_data_dir` — cookies/localStorage 跨 run 持久化
- 默认 `headless=False`(headed) — 让 `request_human_assist` 工具能拉前台让人操作

**单 BrowserManager 绑定单 domain。** 引擎不要中途切换(换引擎=新指纹=site 看作"新设备",可能踢登录);headed↔headless 同 profile 内随意切。
...
**人类介入（1 个）：**
- `request_human_assist(reason)` — 当 agent 识别"我搞不定"(登录页/CAPTCHA/2FA/邮箱验证码)时调,浏览器窗口前置,人在窗口操作,完成后 agent 自己 re-observe 判断状态。Thin wrapper → `src/runtime/human_assist.py` 的 Gateway。LLM 自由文本描述 reason,**不分类型**(单一工具,polymorphism 在人的回答里)。详见本文档 §八。
...
**System Prompt：** 短、稳定。教 agent 怎么想（8 条思维原则，含"遇到 human-only gate 调 request_human_assist"），不含任何站点特定信息。具体内容见 `src/agent/session.py` SYSTEM_PROMPT。
...
**验证：** 给任意目标域名 + 一个 requirement，能跑完多轮 session，World Model 有 locations / observations / Semantic Model / Procedural Model，最终停止。
...
**系统是 domain-agnostic 的。** 没有 hardcoded 目标站点 — agent 应该对任何合法网站工作。

选验证站点的标准:
- SPA + 公开 API + 多种数据路径(确保架构覆盖各种场景)
- 公开数据充足(避免 mission 卡在登录上)
- 数据有清晰的"primary data"语义(便于判定成败)

### MVP 成功标准

给任意 `domain + requirement`,系统能:

1. 启动并连接 DB + Camoufox + LLM
2. ReconPlanner 发起 Session,Agent 自主导航探索
3. World Model 中出现多个 Location(分类/详情/API 端点等)
4. Observations 包含:页面摘要、agent 洞察、提取方法
5. Semantic Model 记录了结构理解和位置间关系
6. Model 包含高层 insight(如"多条数据路径的覆盖关系")
7. `artifacts/{domain}/runs/{run_id}/samples/` 中有真实的 primary data(kind='sample')
...
> 完整决策见对话归档 2026-04-25。下面是落地后的稳定描述。

**定位：** 系统全自主长跑,human assist 是**异常逃生口**(99% 无人介入)。借鉴 Anthropic 立场 "oversight wraps the loop, not pierce it"。

**分层(stable boundary):**

```
Execution Agent
  └─ tool: request_human_assist(reason: str) → {status, message?, next_step_hint}
     └─ Tool 层(thin wiring,call() 只一行)
        └─ HumanAssistGateway 接口(永久稳定)
           └─ TerminalGateway / WebGateway / SlackGateway / ...(实现可换)
              └─ External: 终端 / 浏览器 / 信号文件 / 人
```

**Tool 形态:单工具自由文本** —— 不分 login/captcha/2fa 多工具,polymorphism 在人的回答里。

**State 同步:拉模型** —— Tool 返回的 `next_step_hint` 提示 agent 调 browse 重新观察,不主动 push 状态变更。Agent 用通用 perceive+judge 判断登录态,**不需要 verify_login 专用工具**。
...
**故意 OUT OF SCOPE(MVP):** Loop detection / runaway 兜底(独立话题,见未来 runtime 安全网设计);多渠道并发推送;Web/Slack/Push gateway(接口已支持);跨 session 并发 assist。
