...
```
Episodic Memory（情景记忆）— 做了什么
  = Observations，append-only，原始感知和行动记录
  人类类比：田野笔记本
...
- Agent 执行任务 → 主要读 Procedural Model + Briefing 中的原始 observations
- 人类评估结果 → 读 Semantic Model + Procedural Model
- 所有层 → Episodic 作为证据源和审计追踪

### 关键区分：Data vs Model

| 层 | 性质 | 增长方式 | 更新方式 | 谁维护 |
|---|------|---------|---------|--------|
| Episodic (Observations) | 原始数据 | 无限追加 | append-only | Agent 工具自动写入 |
| Semantic Model | 结构化理解 | 有界 | LLM 全量重写 | Planner LLM |
| Procedural Model | 方法论 | 有界 | LLM 全量重写 | Planner LLM |

Observations 是事实记录——"我在这个页面看到了什么"。Model 是理解——"这个站点的数据是怎么组织的、怎么获取"。
...
- **Append-only**：observations 只追加不修改不删除
- **自由格式**：raw 是 JSONB，不预设 schema
- **工具写入**：Agent 通过 browse/extract/note_insight 写入，不直接操作 DB
...
- **有界**：~3000 chars 上限。超限时 LLM 压缩旧内容，不截断
...
- 我们的两个文档都在 ~3000 chars 以内，远低于这个阈值
...
```
Agent Session 执行中
  ├─ browse/extract/note_insight → Observations（append-only 写入 DB）
  └─ read_world_model → 读取 Semantic Model + Procedural Model + Observations 索引
...
Episodic（原始记录）+ Semantic（站点理解）+ Procedural（方法论）。三层服务不同认知需求，不可合并。

理由：CoALA 框架、AriGraph、Generative Agents 等系统证明双层（episodic + semantic）比单层显著提升 agent 能力。Procedural 独立于 semantic 因为"世界是什么样"和"怎么操作"是不同类型的知识。
