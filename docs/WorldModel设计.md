...
| Semantic / Procedural Model | 精炼理解 | **全量重写** | maintain_model LLM 函数 |
...
Semantic Model 和 Procedural Model 由 `maintain_model` LLM 函数更新——每次 `spawn_execution` 的 session 结束后，**Python 代码自动触发**（不是 Planner 的工具调用）。

```
spawn_execution 内部 pipeline：
  Session 结束 → Python 代码调 maintain_model

maintain_model LLM 函数调用：
  输入：
    - 现有 Semantic Model 全文
    - 现有 Procedural Model 全文
    - 本 session 新增的 observations
    - Session transcript 摘要
  输出：
    - 更新后的 Semantic Model 全文
    - 更新后的 Procedural Model 全文
    - Session summary（给 Planner 的摘要）
    - Model diff（给 Planner 的变化概览）
```

Model 更新是**独立的 LLM 函数调用**，与决策分离。Planner 通过 `read_model()` 工具按需读取最新 Model。
...
第一个 session 之前没有 model。第一次 maintain_model 从零生成两个文档。prompt 中提供文档结构模板和质量要求作为参考。
...
Agent Session 执行中
  ...
  ├─ 单例 Recording Agent（持续运行）：维护 Observations（CRUD）
  │   transcript 增量由 Python 代码推送（Producer-Consumer 模式）
  └─ read_world_model → 两层检索：无参数返回完整 Models，指定 location 返回 Observations

Session 结束 → Python 代码自动执行 pipeline
  │
  ├─ maintain_model（LLM 函数调用，非 agent）
  │    输入：current models + 本 session new observations
  │    输出：new models（写回 DB）+ session summary + model diff
  │
  └─ 返回给 Planner 的 spawn_execution tool_result（Push）
...
4. 第一次 maintain_model 基于旧 model + 新 observations 更新
...
Transcript（不可变原始记录）→ Observations（录制 Agent 维护的知识层）→ Models（maintain_model LLM 函数精炼的理解）。每层有明确的维护者和可变性规则。

理由：CoALA 框架、AriGraph、Generative Agents 等系统证明多层记忆比单层显著提升 agent 能力。Transcript 保证审计追踪；Observations 由录制 Agent 维护（可合并去重，避免数量爆炸）；Models 由 maintain_model LLM 函数全量重写。Procedural 独立于 Semantic 因为"世界是什么样"和"怎么操作"是不同类型的知识。
...
Model 更新由 maintain_model LLM 函数完成，与 Planner 决策分离。

理由：之前设计合并"提炼"和"决策"在同一次 LLM 调用中（evaluate_and_decide）。但这是两个不同抽象层级的工作——合并会导致 LLM 偷懒/乱序。maintain_model 是数据通道（observations → model），由 Python 代码在 spawn_execution 结束后自动触发，不违反硬约束 #7。
