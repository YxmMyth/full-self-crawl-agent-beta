...
> 状态：共识稿 v3（工具集组成+分类+基础设施+宏观设计原则已共识，各工具具体 schema 待讨论）
> 日期：2026-03-28
> 前置：系统架构与信息流.md、WorldModel设计.md
...
解读（think → 这意味着什么？有什么数据？）
  ↓
行动（extract / interact / browse → 做点什么）
  ↓
验证（结果符合预期吗？数据完整吗？）
  ↓
记录（note_insight → 写入 observation）
  ↓
...
- Agent 的关键发现通过 note_insight / browse / extract 写入 World Model
- Context 内丢失的信息可以通过 `read_world_model` 找回
- 前提：agent 的记录习惯要好——system prompt 引导"发现即记录"
...
环境工具（与外部世界交互）：
  browse           — 导航到 URL，观察页面
  interact         — 操作页面元素（click/fill/scroll 等）
  extract          — 在浏览器 context 执行 JS，提取并保存数据
  bash             — 浏览器之外的一切（HTTP、数据处理、文件、代码、搜索）

知识工具（与 World Model 交互）：
  note_insight     — 写入 agent 的高层理解
  read_world_model — 读取已发现的信息

认知工具：
  think            — tool-calling 模式下的推理空间（无副作用）
```
...
| 工具 | 不可替代的原因 |
|------|---------------|
| browse | bash 无法访问渲染后的页面、AX tree、网络捕获 |
| interact | bash 无法点击/填写/滚动浏览器元素 |
| extract | bash 无法在浏览器 session 中执行 JS（继承 cookie/状态） |
| bash | 通用计算——HTTP、数据处理、文件、代码、搜索 |
| note_insight | bash 无法操作内存中的 World Model + DB 同步 |
| read_world_model | 同上 + 需要格式化呈现 |
| think | tool-calling 模式下没有"不做事只想"的出口 |

**不需要独立工具的能力（bash 覆盖）：**

| 曾考虑的独立工具 | 为什么不需要 |
|-----------------|-------------|
| execute_code | `bash("python3 -c '...'")` 或写临时脚本 |
| search_site / web_search | `bash("python3 -c 'from duckduckgo_search import DDGS; ...'")`，低频操作不值得专用工具 |
| HTTP client | `bash("curl ...")` 或 `bash("python3 -c 'import httpx; ...'")`  |
| Read / Write / Glob / Grep | bash `cat`/`echo`/`find`/`grep` 覆盖；我们不是代码编辑场景 |

### 7.2 泛用性验证（共识）

7 个工具覆盖所有非限制场景，无能力空白：

| 场景 | 工具 |
|------|------|
| 浏览页面看结构 | browse |
| 点击/填写/滚动/选择/按键 | interact |
| 后退、hover | interact（action 类型） |
| 从 DOM/嵌入 JSON 提取数据 | extract（JS 执行） |
| 重放捕获的 API | bash（curl） |
| 处理/分析数据 | bash（python3） |
| 获取 robots.txt/sitemap | bash（curl） |
| 搜索站点相关信息 | bash（搜索 API） |
| 记录发现 | note_insight |
| 回顾进展 | read_world_model |
| 先想后做 | think |
| 检查 cookie/storage | extract（JS：`document.cookie`, `localStorage`） |
| 读写中间文件 | bash |

### 7.3 基础设施层（共识——对 agent 透明，不是工具）

以下能力由框架自动处理，agent 不需要感知也不需要操作：

| 能力 | 实现方案 | 优先级 |
|------|---------|--------|
| Dialog 自动处理 | `page.on('dialog')`: auto-accept alert/confirm, auto-dismiss prompt | P0 |
| Cookie banner 自动处理 | `@duckduckgo/autoconsent`（100+ CMP）+ 启发式兜底 | P0 |
| SPA 智能等待 | MutationObserver settle（1.5s 静默阈值）+ 硬超时（15s） | P0 |
| CSP 绕过 | `bypass_csp=True`（browser context 选项） | P0 |
| 导航错误处理 | 捕获 `page.goto()` 异常，返回结构化错误 | P0 |
| 网络请求被动捕获 | `page.on('response')` 在导航前注册，数据通过 browse 返回值呈现 | P0 |
| 新标签检测 | `context.on('page')` 追踪所有新页面 | P0 |
| 交互前遮挡检测 | `elementFromPoint()` 预检 + 遮挡时 JS click 回退 | P0 |
| Service Worker 阻断 | `service_workers='block'` | P1 |
| 权限自动授予 | `context.grant_permissions(...)` | P1 |
| 下载自动接受 | `accept_downloads=True` + `page.on('download')` 保存到 artifacts | P1 |
| 浏览器崩溃恢复 | 检测 CDP 断开 + 自动重连 | P1 |
| 内存管理 | 定期回收 context，关闭不用的 page | P1 |

### 7.4 文件系统设计（共识）

**问题：** 之前 `artifacts/samples/` 堆满不可读的 JSON 文件。

**方案：**

- extract 工具提供 `save_as` 参数，由 agent 控制文件名和路径
- 目录约定：

```
artifacts/
├── samples/          ← extract 存的数据样本（agent 命名）
│   ├── pens/
│   │   ├── threejs_tag_page1.jsonl
│   │   └── pen_detail_abc123.jsonl
│   └── api/
│       └── search_endpoint_response.json
├── workspace/        ← agent 的工作文件（中间结果、脚本）
└── manifest.json     ← 索引（什么文件、从哪来、什么时候）
```

- 暂不加独立 Read/Write 工具，bash 覆盖文件操作
- 如果实际运行中 LLM 用 bash 写文件频繁出错，再考虑加轻量 Write 工具
...
```
interact(
  action: enum   — click, fill, select, press_key, scroll_down, scroll_up, go_back, hover
  target: string — browse 返回的元素编号（如 "3"）。click/fill/select/hover 必需
  value:  string — fill 的文本、select 的选项、press_key 的按键、scroll 的屏数。默认 ""
)
```
...
|--------|--------|-------|------|
| click | 元素编号 | （不用） | `interact("click", "3")` |
| fill | 元素编号 | 输入文本 | `interact("fill", "8", "threejs")` |
| select | 元素编号 | 选项文本 | `interact("select", "10", "Newest")` |
| press_key | （可选焦点元素） | 按键名 | `interact("press_key", "", "Enter")` |
| scroll_down | （不用） | 屏数，默认 "1" | `interact("scroll_down")` |
| scroll_up | （不用） | 屏数，默认 "1" | `interact("scroll_up")` |
| go_back | （不用） | （不用） | `interact("go_back")` |
| hover | 元素编号 | （不用） | `interact("hover", "5")` |

**元素定位：只接受编号，不支持 CSS selector。**
- 编号来自 browse 或上一次 interact 返回的快照
- 编号失效时返回明确错误 + 提示 agent 调 browse 刷新
- 把定位质量问题集中到 browse 的检测逻辑，不在 interact 兜底

**返回值：轻量结果 + 变化信号（不返回完整页面快照）**

interact 和 browse 职责分离（模式 B，与 Manus、Agent-E 对齐）：
- interact 负责：操作 + 检测变化
- browse 负责：深度感知（数据信号、网络捕获、WM 写入）

返回值包含：
```
=== Action Result ===
Action: click [3] (button "Load More")
Result: success

=== Changes Detected ===
URL changed: no
DOM changed: yes — 20 new elements appeared
Hint: New content loaded. Use browse() if you need data signals and API discovery for this page.
```
...
| 情况 | 返回内容 | agent 下一步 |
|------|---------|------------|
| URL 变化（导航） | "Navigated to [new URL]" | 通常调 browse() 做完整分析 |
| DOM 变化（AJAX/modal） | "DOM changed, N new elements" | 可继续 interact 或 browse() |
| 无变化 | "No changes detected" + hint | think 分析原因或换方法 |

**为什么不返回完整页面快照：**
- 调研发现没有系统在交互后做"深度感知"（数据信号检测、网络捕获、WM 写入）
- browse 承担了重职责（Location 创建、Observation 追加、框架检测、嵌入数据检测），interact 不应重复
- 职责清晰：interact 返回信号，agent 根据信号决定是否 browse
- Playwright MCP 的选择性快照、Manus 的 click→view 分离、Agent-E 的变化通知都验证了这种模式

**内部实现（对 agent 透明）：**
- 3 级点击回退链：正常点击 → force click → JS click
- 遮挡检测：elementFromPoint() 预检
- 自动 scroll into view
- fill 内部先 Ctrl+A + Backspace 清空
- DOM 变化检测：MutationObserver（Agent-E 启发）
- URL 变化检测：交互前后 URL 比较

**错误信息设计（可操作的诊断信息）：**

| 错误 | 返回信息 |
|------|---------|
| 编号不存在 | "Element [N] does not exist. Available: [1]-[M]. Page may have changed — use browse() to refresh." |
| 元素已从 DOM 移除 | "Element [N] is no longer in the DOM. Use browse() to get fresh snapshot." |
| 元素被遮挡 | "Element [N] is behind an overlay. A modal may be blocking interaction." |
| 需要 target 但没传 | "Action 'click' requires a target element number." |
| 元素不可交互 | "Element [N] exists but is not interactable (disabled/hidden)." |

**extract 调研结论：**
- 纯 JS page.evaluate，`save_as` 参数控制文件命名
- 质量反馈：null 字段率、记录数、字段预览
- 提取方法层级意识植入 system prompt（嵌入 JSON > API > DOM）

**bash 调研结论：**
- 无状态 subprocess.run，3 参数（command + timeout + working_dir）
- 输出截断 head 4000 + tail 4000，完整输出存文件
- Docker 容器安全边界

**note_insight 调研结论：**
- 2 参数（content 必需 + location 可选），自由格式文本
- 合并 note_relation，关系在 content 中自然描述

**read_world_model 调研结论：**
- 2 参数（section enum + location 模糊匹配）
- 默认返回 Semantic Model + Procedural Model + Location 索引

**think 调研结论：**
- 1 参数（thought），返回 "ok"，no-op
- tau-bench +54.1%，BrowseComp +40.1%，prompt 引导是关键放大器

**interact 质量方向：**
- Action 集：click, fill, scroll_down, scroll_up, select, press_key, go_back, hover
- 变化反馈：交互后返回变化摘要（Agent-E 风格：URL 变化 + 新元素 + 内容变化），省掉 interact→browse 双步
- 内部实现：元素匹配回退链 + 点击回退链（对 agent 透明）
...
**note_insight / read_world_model 质量方向：**
- note_insight 区分于 browse/extract 的自动 observation（高层理解 vs 低层记录）
- read_world_model 支持分 section 过滤 + 位置定向查询
...
| "每个重要发现用 note_insight 记录" | System prompt（跨工具纪律） |
...
### D10: 7 个工具分 3 类——环境/知识/认知

环境工具（browse, interact, extract, bash）与外部世界交互，决定 agent 的行动能力。知识工具（note_insight, read_world_model）与 World Model 交互，决定 agent 的记忆能力。认知工具（think）提供推理空间。三类性质不同，不应混为一谈。

理由：环境工具的设计核心是泛用性（能否覆盖所有场景）和质量（返回值信息密度）。知识工具的核心是认知连贯性。认知工具是 tool-calling 模式的结构性补丁。
...
7 个工具在整个 session 生命周期内始终完整存在。不根据任务类型或阶段动态添加/移除工具。如果某工具在某些场景不适用，通过描述中的使用条件指导 agent。
...
*下一步讨论：extract / bash / note_insight / read_world_model / think 的具体设计确认，Context 管理（§十）。*
