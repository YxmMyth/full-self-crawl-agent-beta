...
> extract 更名为 browser_eval，note_insight 删除（单例 Recording Agent 负责）。
> 详见 `docs/工具重新设计共识.md`。
...
注：Agent 不负责记录——单例 Recording Agent（Producer-Consumer 模式）实时并行维护 Observations，通过 Producer-Consumer 模式接收所有 session 的 transcript 增量。
  ↓
（循环）
```
...
- Context 内丢失的早期信息可以通过 `read_world_model` 找回（单例 Recording Agent 已实时维护）
- Agent 不需要主动记录——它的 context 就是工作记忆，单例 Recording Agent 负责结构化
...
2. Semantic Model 和 Procedural Model 由 maintain_model LLM 函数维护，agent 可读
...
| 遗忘 | 早期发现被 context 删掉 | 工作记忆 | Context microcompact + World Model |
...
- **遗忘** → context microcompact + World Model 外部记忆
...
- **删除 note_insight**——单例 Recording Agent（Producer-Consumer 模式）全权负责 Observation 写入
...
**单例 Recording Agent（Producer-Consumer 模式）：独立于执行 Agent 工具集。** 通过 Producer-Consumer 模式接收所有 session 的 transcript 增量，拥有 4 个专属工具（read/create/edit/delete observations）。Recording Agent 先 read 再改（参考 Claude Code 的 Read/Edit 模式）。Transcript（JSONL）是不可变审计追踪。
...
| "单例 Recording Agent 实时并行记录，需要回忆时调 read_world_model" | System prompt（跨工具纪律） |
...
- 单例 Recording Agent 实时并行维护 Observations → 重要信息已外化到 DB
...
- 我们的结构性优势：单例 Recording Agent + World Model 是天然的外部记忆，比其他系统多一层保障
...
浏览器感知（browse, read_network, browser_eval）、浏览器管理（browser_reset）、页面交互（click, input, press_key, scroll, go_back）、系统执行（bash）、认知辅助（think, read_world_model）。WM 写入由单例 Recording Agent（Producer-Consumer 模式）负责，不是执行 Agent 的工具。
