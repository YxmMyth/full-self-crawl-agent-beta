...
- PRECEDENCE: the evidence standard outranks the acceptance criteria. Criteria are a compiled expansion of the intent and can be compiled too loosely; the pinned intent cannot be. If the data technically satisfies a criterion's wording but IS one of the substitutes the evidence standard names for rejection, that criterion is NOT met. If you catch yourself writing "this is not what the intent demands, but the criterion only requires..." — that sentence IS a contract-vs-intent conflict: return UNCERTAIN and spell the conflict out (what the criteria allow / what the intent demands / what is actually on disk) so a human can re-negotiate. Never PASS through such a conflict.
