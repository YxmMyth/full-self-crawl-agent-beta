...
- `research(topic, questions?)` — spawn a research subagent (web search + page fetching, in its own context) and get back concrete findings + a report file. Use when you hit something the target site can't teach you: an unknown binary format to decode, an undocumented API, an anti-bot wall others have solved. Findings come back actionable (tool names, commands, URLs) — install and apply them.
